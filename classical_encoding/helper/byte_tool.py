    ...
    def unpack_bytes_to_bits(
        ...
        if not end_symbol:  # the bytes are exactly the bits we need
            ...
            for count, bit in enumerate(reversed(last_bits)):
                ...
                if matching_bits == end_bits:
                    break
            else:
                print(f"{end_bits=}, {last_bits=}")
...
def test_packer_with_bits():
    ...
    packer = BytePacker()
    source: Bits = Bits.from_bools(
        bool(randint(0, 1)) for _ in range(randint(100, 1000))
    )
    packer.pack_bits(source)
    end_symbol = Bits.from_bools([False])
